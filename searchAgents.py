    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not isWall:
                cornerCount = 0
                for corner in self.corners:
                    if (next_x, next_y) == corner:
                        break
                    cornerCount += 1
                if cornerCount < 4:
                    nextCornerVisited[cornerCount] = True
                next_state = ((next_x, next_y), nextCornerVisited)
                cost = self.costFn(x, y)
...
def cornersHeuristic(state, problem):
    ...
    cornerNotVisited = []
    for i in range(4):
        if corners[i] not in cornerVisited:
            cornerNotVisited.append(corners[i])
    currentPosition = node
    while (len(cornerNotVisited) != 0):
        distance, corner = min([(util.manhattanDistance(currentPosition, corner), corner)
